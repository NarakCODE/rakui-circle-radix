...
function getAvatarImageUrl(person: AssignedPerson) {
  ...
  return (
    <div className="flex -space-x-2">
      {people.map((person) => (
        ...
        >
          <AvatarImage
            src={person.image ?? getAvatarImageUrl(person)}
            alt={person.name}
          />
...
export default function TopCompaignsTable() {
  return (
    <Card>
      <CardHeader>
            ...
            {data.map((group) => (
              <Fragment key={group.name}>
                <TableRow className="bg-muted/50 hover:bg-muted/50">
                  <TableCell colSpan={7} className="py-2 pl-4 font-semibold">
                    {group.name}
                ...
                {group.items.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell className="text-center">
                      <Checkbox aria-label={`Select ${item.task}`} />
                    </TableCell>
